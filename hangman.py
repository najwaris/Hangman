...
class Hangman:
    ...
    def __init__(self):
        ...
        This method builds the GUI layout, creates the GUI window, initializes the canvas,
        starts the game, starts the main event loop, and initializes the game and player statistics.
        """
        layout = [
            [   
                # Build the frames for the canvas and the letter buttons
                self._build_canvas_frame(),
                self._build_letters_frame(),
            ],
            [
                # Build the frame for the guessed word
                self._build_guessed_word_frame(),
            ],
            [
                # Build the frame for the action buttons
    ...
    def _build_canvas_frame(self):
        # Create the canvas frame for drawing the hangman
        return sg.Frame(
            "Hangman",
            [
                [
            ...
            # Set the font for the canvas frame
            font="Any 20",
        )

    def _build_letters_frame(self):
        # Create the frame for displaying the letter buttons
        letter_groups = [
    ...
    def _build_guessed_word_frame(self):
        # Create the frame for displaying the guessed word
        return sg.Frame(
            "",
            [
                [
    ...
    def _build_action_buttons_frame(self):
        # Create the frame for action buttons (New, Restart, Quit)
        return sg.Frame(
            "",
            [
                [
    ...
    def _draw_hanged_man(self):
        # Draw the hanged man based on the number of wrong guesses
        head = (100, 290)
    ...
    def _build_guessed_word(self):
        # Build the string representation of the guessed word
        current_letters = []
